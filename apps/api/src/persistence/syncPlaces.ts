 ...
 * current export's places at all — i.e. places removed from Google Maps
 * since the last import.
 *
 * Deletion scope is deliberately based on `allPlaces` (the full parsed CSV
 * set, derived locally via extractPlaceIdFromUrl — no network involved) and
 * NOT on `resolvedPlaces` (only the subset that successfully resolved this
 * run). A place that's still in the export but hit a transient Places API
 * failure this run must not look the same as one you actually removed —
 * it should just keep its stale data, not get deleted.
 *
 * Refuses to run at all if the current export resolves to zero place IDs —
 * an empty set almost certainly means a misconfigured/empty directory or a
 * parsing bug, not that every saved place was actually removed. Without
 * this guard, an empty set would otherwise match (and delete) every row.
 */
export async function syncPlaces(
  ...
  if (currentPlaceIds.length === 0) {
    throw new Error(
      "Refusing to sync: the current export resolved to zero place IDs, " +
        "which would delete every saved place. Check the import directory.",
    );
  }
  ...
  try {
    ...
    const { rowCount: deleted } = await client.query(
      `delete from places where place_id <> all($1::text[])`,
      [currentPlaceIds],
    );
