 ...
 * my_places_test (see vitest.env.ts), never the real database, so this is
 * safe to call freely. Both tables in one statement so Postgres handles the
 * places → users foreign key itself, without needing CASCADE.
 */
